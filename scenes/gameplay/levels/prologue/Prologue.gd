...
func _on_scene_activated() -> void:
	...
	# Verificar estado do prólogo através do StateManager
	...
	if not state_manager:
		printerr("Prologue.gd: StateManager não encontrado! Não é possível verificar o estado da introdução.")
	...
	print("Prologue: Verificando flag 'prologue_completed' = ", state_manager.prologue_completed)
	
	if state_manager.prologue_completed == true:
...
func _finalizar_dialogo_da_introducao() -> void:
	...
	if state_manager:
		state_manager.prologue_completed = true
		print("Prologue: Flag 'prologue_completed' definida como true.")
...
func _prosseguir_apos_dialogo() -> void:
	...
	if state_manager:
		...
		printerr("Prologue: Não foi possível encontrar StateManager para mudar para o estado Gameplay.")
